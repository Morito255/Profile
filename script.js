...
async function syncViewCount() {
  const storageKey = 'moro-profile-viewed-this-session';
  const shouldIncrement = !sessionStorage.getItem(storageKey);
  ...
  try {
    ...
    if (shouldIncrement) sessionStorage.setItem(storageKey, 'true');
  } catch {
